...
with open('alldata.csv','a+',encoding='utf-8') as target:
    ...
    for province in provinces:
        with open(os.getcwd() + "/{}_info.txt".format(province), 'r', encoding='utf-8') as f:
            data = f.read().split('\n')
            if data[-1] == '':
                data.pop()
            for item in data:
                ...
                try:
                    ...
                    print("xxx" + item)
                mouth = i[2].split('年')[0] + '-' + i[2].split('年')[1].split('月房价')[0]
                mouth = str(mouth)
                province = i[0]
                city = i[1]
                citylevel = '无'
                longitude = '0'
                twist = '0'
                houseprice = i[3].split('元')[0]
                try:
                    proportion = str(round(float(i[4].split('%')[0])/100,4))
                    inc = i[4].split('%')[1]
                    if inc == '↑':
                        inc_2 = '上升'
                    else:
